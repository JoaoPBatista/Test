---
summary: Use Service modules and Service applications to design a service-oriented architecture.
...
In OutSystems, you can use **Service modules** in your Web or Mobile application to enforce the separation of concerns and encapsulate the core service functionality.
...
You should use Web or Mobile modules to implement UI related elements. 
...
* **Expose functionality to an external system or a different OutSystems infrastructure:** You can implement the logic of your service using [REST](../../extensibility-and-integration/rest/expose-rest-apis/intro.md) or [SOAP](../../extensibility-and-integration/soap/expose-a-soap-web-service.md) integrations in your Service modules.

* **Expose a service to be reused by other modules or applications inside the same OutSystems environment:** You can implement the logic of your service in a tightly-coupled way using **Server Actions**, or you can do it in a loosely-coupled way using **Service Actions**.
...
The Server Action logic is executed just as if it is defined in the consumer module, running in process with the same request and transaction.
...
In OutSystems, a **Service Action** is a remote call to another process, but its usage is very similar to public Server Actions. 
...
**Impact analysis**
:   OutSystems calculates the impact of changing the signature of a Service Action and shows it in the Manage Dependencies window. This helps you to decide whether you need to create a new version of your service or if the changes in your service have no impact on the consumers.
...
Using Service Actions implies executing logic through remote calls, which can have significant performance penalties when compared to running the same logic in process. Therefore, your logic must take into account the amount of data passed through the remote requests. 
...
| **Service communication** | Consumer and producer modules run in process. | Consumer and producer modules run in different processes. |
...
Using **Server Actions** to expose Customer functionality simplifies the logic of the Order Management application, since the logic runs in process with the same transaction. However, changes in the Customer service may require the deploy of the Order Management application too.

As long as these modules have the **same release cycle**, keeping these modules strongly-coupled will not have negative impacts in the deployment phase, while positively impacts the development speed.
